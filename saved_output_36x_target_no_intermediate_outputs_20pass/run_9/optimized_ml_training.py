...
1. Vectorized custom_normalize using efficient distance computation
2. Vectorized SlowCustomActivation using torch operations
3. Fully vectorized gradient feature extraction
4. Optimized numpy operations
"""
...
class ComplexFeatureExtractor:
    ...
    def extract_statistical_features(self, data):
        """Extract statistical features from 2D data - fully vectorized."""
        mean = np.mean(data, axis=1)
        std = np.std(data, axis=1)
        max_val = np.max(data, axis=1)
        min_val = np.min(data, axis=1)
        median_abs_dev = np.median(np.abs(data - mean[:, np.newaxis]), axis=1)
        
        features = np.column_stack([mean, std, max_val, min_val, median_abs_dev])
        return features

    def extract_gradient_features(self, data):
        """Extract gradient-based features - fully vectorized."""
        n_samples = data.shape[0]
        data_reshaped = data.reshape(n_samples, 28, 28)
        
        # Vectorized gradient computation
        grad_x = np.abs(np.diff(data_reshaped, axis=2))
        grad_y = np.abs(np.diff(data_reshaped, axis=1))
    ...
    def extract_frequency_features(self, data):
        ...
        for i in range(n_samples):
            fft = np.fft.fft2(data_reshaped[i])
    ...
    def custom_normalize(self, features):
        """
        Custom normalization using distance-based scaling - highly optimized.
        Uses efficient vectorized computation with minimal memory overhead.
        """
        n_samples = features.shape[0]
        d_samples = features.shape[1]
        
        # Compute pairwise squared distances more efficiently
        # ||a - b||^2 = ||a||^2 + ||b||^2 - 2*a*b
        feature_norms_sq = np.sum(features ** 2, axis=1)  # (n,)
        
        # Compute all pairwise distances
        # Using formula: ||features[i] - features[j]||^2 = norms[i] + norms[j] - 2*dot(features[i], features[j])
        dot_product = np.dot(features, features.T)  # (n, n)
        distances_sq = feature_norms_sq[:, np.newaxis] + feature_norms_sq[np.newaxis, :] - 2 * dot_product
        distances_sq = np.maximum(distances_sq, 0)  # Handle numerical errors
        distances = np.sqrt(distances_sq)
        
        # Compute scale for each sample (median distance + epsilon)
...
class SlowCustomActivation(nn.Module):
    """
    Custom activation function - optimized.
    """

    def forward(self, x):
        tanh_x = torch.tanh(x)
        return torch.where(x > 0, x * 0.9 + tanh_x * 0.1, tanh_x * 0.5)
