...
class ComplexFeatureExtractor:
    ...
    def extract_gradient_features(self, data):
        ...
        # Compute gradients using numpy operations
    ...
    def custom_normalize(self, features):
        """
        Optimized custom normalization using vectorized distance computation.
        """
        ...
        dist_sq = sq_norms + sq_norms.T - 2 * features @ features.T
        dist_sq = np.maximum(dist_sq, 0)  # Ensure non-negative due to numerical errors
        distances = np.sqrt(dist_sq)
        
        # Compute scale for each sample as median distance
