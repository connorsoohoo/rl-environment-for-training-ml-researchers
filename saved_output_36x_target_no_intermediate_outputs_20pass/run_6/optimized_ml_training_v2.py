...
2. Further vectorization of statistical features
...
class ComplexFeatureExtractor:
    ...
    def extract_frequency_features(self, data):
        """Extract frequency domain features - vectorized with batch processing."""
        n_samples = data.shape[0]
        features = np.zeros((n_samples, 4), dtype=np.float32)
        ...
        # Compute FFT for all images at once using rfft2d for efficiency
        fft = np.fft.rfft2(img_reshaped)
        fft_magnitude = np.abs(fft)
        
        # Vectorized feature extraction
        fft_flat = fft_magnitude.reshape(n_samples, -1)
        features[:, 0] = np.mean(fft_flat, axis=1)
        features[:, 1] = np.std(fft_flat, axis=1)
        features[:, 2] = np.max(fft_flat, axis=1)
        # Vectorized percentile using np.partition for efficiency
        for i in range(n_samples):
            features[i, 3] = np.partition(fft_flat[i], int(0.95 * fft_flat.shape[1]))[int(0.95 * fft_flat.shape[1])]
