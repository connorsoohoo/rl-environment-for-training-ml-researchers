...
class ComplexFeatureExtractor:
    ...
    Uses vectorized operations for significant speedup.
    """
    ...
    def custom_normalize(self, features):
        """
        Custom normalization using vectorized distance-based scaling.
        MAJOR OPTIMIZATION: Replace pairwise distance loop with vectorized computation.
        """
        # Vectorized pairwise distance computation
        # ||a - b||^2 = ||a||^2 - 2ab + ||b||^2
        n_samples = features.shape[0]
        sq_norms = np.sum(features ** 2, axis=1, keepdims=True)  # Shape: (n_samples, 1)
        
        # Compute all pairwise distances at once
        # distances_sq[i, j] = sum((features[i] - features[j])^2)
        distances_sq = sq_norms - 2 * features @ features.T + sq_norms.T
        # Ensure non-negative (handle numerical errors)
        distances_sq = np.maximum(distances_sq, 0)
        distances = np.sqrt(distances_sq)
        
        # Compute scale for each sample (median distance)
...
class SlowCustomActivation(nn.Module):
    ...
    Uses vectorized operations instead of element-by-element loops.
    """

    def forward(self, x):
        # Vectorized computation
        # For positive values: val * 0.9 + tanh(val) * 0.1
        # For non-positive: tanh(val) * 0.5
