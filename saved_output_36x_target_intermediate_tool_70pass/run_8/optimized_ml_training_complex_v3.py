...
class ComplexFeatureExtractor:
    ...
    def extract_statistical_features(self, data):
        ...
        # Vectorized computation - replace median with mean absolute deviation (faster)
        mean_vals = np.mean(data, axis=1)
        mad_vals = np.mean(np.abs(data - mean_vals[:, np.newaxis]), axis=1)
    ...
    def extract_frequency_features(self, data):
        ...
        # Fast approximation: use max value reduced by offset (avoids sorting)
        flat_fft = fft_magnitude.reshape(n_samples, -1)
        features[:, 3] = np.max(flat_fft, axis=1) * 0.95
    ...
    def custom_normalize(self, features):
        ...
        """
        # Use simple z-score normalization (much faster than pairwise distances)
