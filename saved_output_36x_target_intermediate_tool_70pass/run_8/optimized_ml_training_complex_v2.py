...
class ComplexFeatureExtractor:
    ...
    def extract_frequency_features(self, data):
        """Extract frequency domain features - OPTIMIZED (removed expensive percentile)."""
        n_samples = data.shape[0]
        features = np.zeros((n_samples, 4), dtype=np.float32)
        ...
        # Instead of percentile (which is slow), use a faster approximation: quantile or max value
        # Use 90th percentile approximation via sorting and indexing
        flat_fft = fft_magnitude.reshape(n_samples, -1)
        # Use partition for faster percentile approximation (O(n) instead of O(n log n))
        partition_idx = int(0.95 * flat_fft.shape[1])
        features[:, 3] = np.partition(flat_fft, partition_idx, axis=1)[:, partition_idx]
