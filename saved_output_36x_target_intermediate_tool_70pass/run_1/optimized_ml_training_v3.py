...
class ComplexFeatureExtractor:
    ...
    def extract_statistical_features(self, data):
        ...
        median_vals = np.median(np.abs(data - mean_vals[:, np.newaxis]), axis=1)
        
        features = np.column_stack([mean_vals, std_vals, max_vals, min_vals, median_vals])
        return features
    ...
    def extract_gradient_features(self, data):
        ...
        # Vectorized gradient computation using numpy diff
        grad_x = np.abs(np.diff(images, axis=2))  # Shape: (n_samples, 28, 27)
        grad_y = np.abs(np.diff(images, axis=1))  # Shape: (n_samples, 27, 28)
        
        # Compute statistics for each sample - use axis tuple for efficiency
        mean_gx = np.mean(grad_x.reshape(n_samples, -1), axis=1)
        std_gx = np.std(grad_x.reshape(n_samples, -1), axis=1)
        mean_gy = np.mean(grad_y.reshape(n_samples, -1), axis=1)
        std_gy = np.std(grad_y.reshape(n_samples, -1), axis=1)
        
        features = np.column_stack([mean_gx, std_gx, mean_gy, std_gy])
        return features
    ...
    def extract_frequency_features(self, data):
        ...
        mean_fft = np.mean(fft_flat, axis=1)
        std_fft = np.std(fft_flat, axis=1)
        max_fft = np.max(fft_flat, axis=1)
        perc_fft = np.percentile(fft_flat, 95, axis=1)
        
        features = np.column_stack([mean_fft, std_fft, max_fft, perc_fft])
        return features

    def custom_normalize(self, features):
        """
        Custom normalization using fast scaling.
        Uses norm-based scaling which is O(n) instead of O(n²).
        """
        ...
        # Normalize by dividing each sample by its norm
