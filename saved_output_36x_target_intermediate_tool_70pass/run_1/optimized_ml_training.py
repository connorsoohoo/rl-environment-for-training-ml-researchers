...
class ComplexFeatureExtractor:
    ...
    def custom_normalize(self, features):
        ...
        Uses efficient computation to replace O(n²) pairwise distance calculation.
        """
        # Efficient vectorized approach
        n_samples = features.shape[0]
        
        # Compute all pairwise squared differences at once using broadcasting
        # ||features[i] - features[j]||^2 for all pairs
        diff = features[:, np.newaxis, :] - features[np.newaxis, :, :]  # (n, n, d)
        sq_dist = np.sum(diff ** 2, axis=2)  # (n, n)
        dist = np.sqrt(sq_dist + 1e-10)  # Avoid sqrt(0)
        
        # Compute median distance for each sample
        scales = np.median(dist, axis=1) + 1e-8
