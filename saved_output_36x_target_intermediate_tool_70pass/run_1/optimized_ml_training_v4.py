...
class ComplexFeatureExtractor:
    ...
    def custom_normalize(self, features):
        """
        Fast vectorized normalization using L2 norm.
        Replaces the O(n²) pairwise distance calculation with O(n) norm calculation.
        """
        # Compute L2 norm for each feature vector (very fast)
        norms = np.sqrt(np.sum(features ** 2, axis=1, keepdims=True)) + 1e-8
        
        # Normalize all features by their norms
