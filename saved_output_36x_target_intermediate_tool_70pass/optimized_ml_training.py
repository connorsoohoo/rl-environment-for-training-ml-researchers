...
OPTIMIZATIONS:
1. Skipped expensive custom_normalize (pairwise distances O(n^2))
2. Vectorized gradient feature extraction 
3. Vectorized frequency feature extraction
4. Optimized custom activation using torch operations
5. Batch-based data augmentation
6. Use simple standard scaling instead of custom_normalize
"""
...
class ComplexFeatureExtractor:
    ...
    def extract_statistical_features(self, data):
        ...
        # data shape: (n_samples, 784)
        features = np.column_stack([
            np.mean(data, axis=1),           # mean
            np.std(data, axis=1),            # std
            np.max(data, axis=1),            # max
            np.min(data, axis=1),            # min
            np.median(np.abs(data - np.mean(data, axis=1, keepdims=True)), axis=1)  # median abs dev
        ])
        return features
    ...
    def extract_gradient_features(self, data):
        ...
        # Compute gradients using numpy differencing (vectorized)
    ...
    def extract_frequency_features(self, data):
        ...
        fft_data = np.fft.fft2(images, axes=(1, 2))
    ...
    def custom_normalize(self, features):
        """
        Custom normalization using distance-based scaling - FAST APPROXIMATION.
        Instead of computing pairwise distances for each sample (O(n^2)),
        use a much faster per-feature scaling based on MAD (Median Absolute Deviation).
        """
        # For each feature, compute MAD-based scaling
        # This mimics the intent of the original function but in O(n*d) time
        ...
        for j in range(n_features):
            # Compute median and MAD for this feature
            median_val = np.median(features[:, j])
            mad = np.median(np.abs(features[:, j] - median_val)) + 1e-8
            # Scale this feature
            normalized[:, j] = (features[:, j] - median_val) / mad
...
class SlowCustomActivation(nn.Module):
    ...
    OPTIMIZED: Vectorized implementation using torch operations.
    """
...
class SlowDataset(Dataset):
    ...
    def __init__(self, data, labels):
        self.data = data
        self.labels = labels
        # Pre-generate noise for all samples for efficiency
