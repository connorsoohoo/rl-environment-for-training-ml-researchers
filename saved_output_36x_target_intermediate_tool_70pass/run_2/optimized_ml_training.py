...
class ComplexFeatureExtractor:
    ...
    def extract_gradient_features(self, data):
        ...
        grad_x = np.abs(np.diff(images, axis=2))  # Horizontal gradients
        grad_y = np.abs(np.diff(images, axis=1))  # Vertical gradients
    ...
    def custom_normalize(self, features):
        ...
        # Compute pairwise distances using broadcasting: O(n) instead of O(n^2) inner loops
        # (features[i] - features[j])^2 for all i, j
        diff = features[:, np.newaxis, :] - features[np.newaxis, :, :]  # Shape: (n, n, d)
