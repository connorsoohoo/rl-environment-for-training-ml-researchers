"""
Complex ML Training Pipeline - OPTIMIZED VERSION V4
...
class ComplexFeatureExtractor:
    ...
    def extract_gradient_features(self, data):
        ...
        # Compute statistics without padding (more efficient)
        features = np.column_stack([
    ...
    def extract_frequency_features(self, data):
        ...
        # Use real FFT for better performance since we're dealing with real data
    ...
    def custom_normalize(self, features):
        ...
        Approximation: sample-based median instead of full pairwise distances.
        """
        n_samples = features.shape[0]
        
        # Use sample-based approach: subsample to compute distances for efficiency
        # For large datasets, we can approximate by computing on a subset
        sample_size = min(500, n_samples)  # Use up to 500 samples
        sample_indices = np.random.choice(n_samples, sample_size, replace=False)
        sample_features = features[sample_indices]
        
        # Compute squared norms for samples
        sq_norms_sample = np.sum(sample_features ** 2, axis=1, keepdims=True)
        
        # Compute pairwise squared distances using broadcasting
        dot_product_sample = np.dot(sample_features, sample_features.T)
        sq_distances_sample = sq_norms_sample + sq_norms_sample.T - 2 * dot_product_sample
        sq_distances_sample = np.maximum(sq_distances_sample, 0)
        distances_sample = np.sqrt(sq_distances_sample)
        
        # Compute median distance for samples
        median_distance = np.median(distances_sample)
        
        # Use global median for all samples (approximate)
        scale = median_distance + 1e-8
